...
> **Level 6 gate closed by project-owner risk acceptance on 2026-07-31.** The live matrix was run by the implementation's author, not independently re-run, and no restricted gateway-log correlation was performed. The owner accepted those auditability gaps after two implementation reviews and ratified the three Phase 1 answers below as settled for Phases 2 and 3. See `.claude/PRPs/reviews/bonzai-phase-1-level-6-closure.md`.
...
| Claude session cannot resume under a different Bonzai Key                            | Settled (Level 6 closed with accepted gaps) | **Settled by Phase 1 and ratified at Level 6 closure (2026-07-31): resume under a replacement key works.** The owner accepted that the matrix was not independently re-run and was not correlated against restricted gateway logs. Residual risk moves to context transfer — resume replays the local transcript under the new credential, so a key change across billing contexts must start a fresh session. |
...
| Resume after key replacement (Phase 1 evidence, Level 6 ratified) | Preserve the resume cursor for same-context key rotation; start a fresh session when the key change represents a different client | Always resume, always start fresh, summarize old history           | Validated live and ratified at Level 6 closure on 2026-07-31: replacement-key resume returned the same session ID and recovered prior context, while an invalid or absent credential still failed. Because resume replays the local transcript to the gateway under the new credential, resuming across different billing contexts would transmit one client's context under another's identity. |
| Authorization error UX (Phase 1 evidence, Level 6 ratified)       | Add dedicated Bonzai auth-error mapping plus a retry/deadline cap                                                                 | Rely on the existing generic provider error                        | Missing credentials project as a completed turn showing only `Claude runtime stream failed.`, and invalid credentials never produce a terminal error. Generic propagation is insufficient and misleading.                                                                                                                                                                                        |
...
_Status: ACTIVE - Phase 1 evidence settled; Phases 2-7 pending_
