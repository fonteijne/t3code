...
| Claude session cannot resume under a different Bonzai Key                            | Retired    | **Resolved by Phase 1 (2026-07-31): resume under a replacement key works.** Residual risk moves to context transfer — resume replays the local transcript under the new credential, so a key change across billing contexts must start a fresh session.              |
...
| Resume after key replacement (Phase 1 evidence) | Preserve the resume cursor for same-context key rotation; start a fresh session when the key change represents a different client | Always resume, always start fresh, summarize old history           | Validated live 2026-07-31: replacement-key resume returned the same session ID and recovered prior context, while an invalid or absent credential still failed. Because resume replays the local transcript to the gateway under the new credential, resuming across different billing contexts would transmit one client's context under another's identity. |
| Authorization error UX (Phase 1 evidence)       | Add dedicated Bonzai auth-error mapping plus a retry/deadline cap                                                                 | Rely on the existing generic provider error                        | Missing credentials project as a completed turn showing only `Claude runtime stream failed.`, and invalid credentials never produce a terminal error. Generic propagation is insufficient and misleading.                                                                                                                                                     |
